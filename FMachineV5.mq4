  ...
  // HighMax=0.0;
   ...
   for (int i=Bars-2;  i>1; i--){
      ...
      if (ExisteLiquidezHigh(i)==true){
         ...
         iStoreHL=iStoreHL+1;//avanzar el siguiente punto donde almacenar
         //Mostrar Lectura
         mostrarlectura();
      }
     if (foundH>0){ 
          Temp_H[foundH-1]=StoreHL[iStoreHL-1];  //Almacenar en array Temporal H
          HighMax = findmaximo(Temp_H,foundH-1); //Buscar el valor maximo en array
          if (foundH>=2){
      ...
      if (foundL>0){ 
          Temp_L[foundL-1]=StoreHL[iStoreHL-1];  //Almacenar en array Temporal L       
          LowMin= findminimo(Temp_L,foundL-1); //Buscar el valor minimo en array
          if (foundL>=2){
             LecturaString("",120,70,"PX11",  StringConcatenate("L-L igual."));  
          }   
      }   
      if (foundL<=0){
          LecturaString("",120,70,"PX11",  StringConcatenate("L-L difer."));  
          resetTemp_L();
          LowMin=0.0;
      }      
       mostrarlectura();       
...
void mostrarlectura(){
   ...
   LecturaPrecio(foundH,5,90,"p3",  StringConcatenate("Found-H: "),0);  
   LecturaPrecio(foundL,5,70,"p4",  StringConcatenate("Found-L: "),0);   
   if(iStoreHL>0){
   ...
   int cy=460;
    ...
    LecturaPrecio(Temp_L[0],5,cy,"T0",  StringConcatenate("Temp_L[0]:"),5);        
    LecturaPrecio(Temp_L[1],5,cy+20,"T1",  StringConcatenate("Temp_L[1]:"),5);  
    LecturaPrecio(Temp_L[2],5,cy+40,"T2",  StringConcatenate("Temp_L[2]:"),5);  
    LecturaPrecio(Temp_L[3],5,cy+60,"T3",  StringConcatenate("Temp_L[3]:"),5);  
    LecturaPrecio(Temp_L[4],5,cy+80,"T4",  StringConcatenate("Temp_L[4]:"),5);   
    LecturaPrecio(LowMin,5,cy+100,"T5",  StringConcatenate("LowMin:"),5); 

    LecturaPrecio(Temp_H[0],160,cy,"X0",  StringConcatenate("Temp_H[0]:"),5);        
    LecturaPrecio(Temp_H[1],160,cy+20,"X1",  StringConcatenate("Temp_H[1]:"),5);  
    LecturaPrecio(Temp_H[2],160,cy+40,"X2",  StringConcatenate("Temp_H[2]:"),5);  
    LecturaPrecio(Temp_H[3],160,cy+60,"X3",  StringConcatenate("Temp_H[3]:"),5);  
    LecturaPrecio(Temp_H[4],160,cy+80,"X4",  StringConcatenate("Temp_H[4]:"),5);   
    LecturaPrecio(HighMax,160,cy+100,"X5",  StringConcatenate("HighMax:"),5);              
...
bool igualTypeLL(){
   ...
   if (iStoreHL>2){
      ...
      if (StoreHL_Type[iStoreHL-1]=="L"){
         if (StoreHL[iStoreHL-1]==StoreHL[iStoreHL-2]){
            result=true;
         }
      }
...
double findminimo(double &temp[],int size){
   double minimo=temp[0];
...
double findmaximo(double &temp[],int size){
   double maximo=0.0;
   for(int i=0;i<=size;i++){
      if(temp[i]>maximo){
         maximo=temp[i];
